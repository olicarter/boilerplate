...
test('shows "sign in to vote" when logged out', async ({ page, asAlice }) => {
  const topic = await createTopic(page.request, 'Environment');
  const proposal = await createProposal(page.request, topic.id, 'Ban plastics');

  // Open a fresh context without Alice's session
  const anonPage = await page.context().browser()!.newPage();
  await anonPage.goto(`/proposals/${proposal.id}`);
  await expect(anonPage.getByText('Please sign in to vote')).toBeVisible();
...
test('delegation-resolved tally counts delegated votes', async ({ page, asAlice, bob, request }) => {
  ...
  await page.request.post('http://localhost:5173/api/delegations', {
    data: { id: crypto.randomUUID(), delegator_id: asAlice.id, delegate_id: bob.id, topic_id: null },
  });
