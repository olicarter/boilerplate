...
test('delete button not shown for other users comments', async ({ page, asAlice, bob, request }) => {
  ...
  await request.post(`http://localhost:5173/api/proposals/${proposal.id}/comments`, {
...
test('sign in prompt shown when not authenticated', async ({ page, bob, request }) => {
  const topic = await createTopic(request, 'Policy');
  const proposal = await createProposal(request, topic.id, 'Public policy');

  await page.goto(`/proposals/${proposal.id}`);
  await expect(page.getByText('Sign in to join the discussion.')).toBeVisible();
