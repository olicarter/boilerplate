...
test('cannot vote on a closed proposal', async ({ page, asAlice }) => {
  ...
  const res = await page.request.post(`http://localhost:5173/api/votes`, {
...
test('non-author does not see management actions', async ({ page, asAlice, bob }) => {
  ...
  const res = await page.request.post(`http://localhost:5173/api/proposals/${proposal.id}/close`);
...
test('author can reopen a closed proposal via API', async ({ page, asAlice }) => {
  ...
  const res = await page.request.post(`http://localhost:5173/api/proposals/${proposal.id}/reopen`);
  expect(res.status()).toBe(201);
  const body = await res.json();
...
test('author can withdraw a proposal via API', async ({ page, asAlice }) => {
  ...
  const res = await page.request.post(`http://localhost:5173/api/proposals/${proposal.id}/withdraw`);
  expect(res.status()).toBe(201);
  const body = await res.json();
...
test('withdrawn proposal shows Withdrawn badge on list', async ({ page, asAlice }) => {
  ...
  await page.request.post(`http://localhost:5173/api/proposals/${proposal.id}/withdraw`);
...
test('API rejects a delegation that would create a cycle', async ({ page, asAlice, bob }) => {
  // Alice → Bob
  const res1 = await page.request.post(`http://localhost:5173/api/delegations`, {
    data: { id: crypto.randomUUID(), delegator_id: asAlice.id, delegate_id: bob.id },
  });
  expect(res1.status()).toBe(201);

  // Bob → Alice (would create a cycle)
  const res2 = await page.request.post(`http://localhost:5173/api/delegations`, {
    data: { id: crypto.randomUUID(), delegator_id: bob.id, delegate_id: asAlice.id },
  });
...
test('API rejects self-delegation', async ({ page, asAlice }) => {
  const res = await page.request.post(`http://localhost:5173/api/delegations`, {
